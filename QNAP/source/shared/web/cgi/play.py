...
class ASPlayer:
    ...
    def __PlayTask(self, task, offs):
        ...
        logger.debug(" volume start = %d, volume end = %d, delta = %f. [%f , %f]" % (volumeStart, volumeEnd, volumeDelta, tmpl, volt))
        ...
        while (True):
            ...
            if (elapsed < (volt + 1)):
                curVolume = int(round(volumeDelta * elapsed) + volumeStart)
                logger.debug(" volume: %d" % (curVolume))
