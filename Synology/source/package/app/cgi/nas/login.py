...
# login with non-encrypted login api call
# (not documentewd method)
# see https://rileykidd.com/2016/01/12/synology-nas-dsm-5-2-remote-code-execution-rce/
def login_internal(login, password):
    logger.info("[login.login_internal]")
    URL = "/webman/login.cgi?enable_syno_token=yes"
    data = 'username=' + login + '&passwd=' + password + '&OTPcode=&__cIpHeRtExT=&isIframeLogin=no&enable_device_token=no'
    ...
    if resdata['success'] == 1:
        token = resdata['SynoToken']
        naswebapi.setToken(token)
        ...
        for item in cookie_arr:
            ...
            if item_arr[0] == "id":
                ...
                naswebapi.setCookie("stay_login=1; id=" + sid)
        return ""
    else:
