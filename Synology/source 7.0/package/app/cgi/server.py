...
class Core(object):
    ...
    def run(self):
        ...
        try:
            ...
            logger.debug("players cache - fill memory cache")
            self.updatePlayers()
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 2:
        if (sys.argv[1] == "-i"): #interactive
            ...
            core = Core(False)
            run = daemon.runner.DaemonRunner(core)
            run.do_action() # start|stop|restart as sys.argv[1]
    else:
        print ("Usage: %s start|stop|restart or -i for console mode" % sys.argv[0])
        sys.exit(1)
else:
